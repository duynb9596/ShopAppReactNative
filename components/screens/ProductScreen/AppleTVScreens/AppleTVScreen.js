...
export default class AppleTVScreen extends React.Component {
  ...
  _loadData()
  {
    firebaseApp.database().ref('shopapp/list_ProductType/Apple TV/product/Apple TV (4th generation)/').once('value', (product) => {
      console.log(product)
      this.setState({
    ...
    firebaseApp.database().ref('shopapp/list_ProductType/Apple TV/product/Apple TV (4th generation)/model/image').once('value', (image) => {
      this.setState({
        image: image.val(),
      })
    })
    firebaseApp.database().ref('shopapp/list_ProductType/Apple TV/product/Apple TV (4th generation)/price/').once('value', (price) => {
      this.setState({
        fromprice: parseInt(price.val()),
        storageprice: parseInt(price.val())
      })
    }).then(() => {
      firebaseApp.database().ref('shopapp/list_ProductType/Apple TV/product/Apple TV (4th generation)/model/list_Storage/').once('value', (childStorageSnapshot) => {
        ...
        console.log(this.state.list_Storage)
        ...
        this.setState({
          name: 'Apple TV (4th generation)' + ' ' + this.state.storage
        })
      })
    })    
  }
  ...
  _writeData(){
    firebaseApp.database().ref('shopapp/list_ProductType/Apple TV/product/Apple TV (4th generation)/model/image').set('https://firebasestorage.googleapis.com/v0/b/shopapp-8541a.appspot.com/o/AppleTV%2FAppleTV.jpg?alt=media&token=ee66c34d-37ad-46d0-81ba-84a48b91396d')
  }

  async _addToBag()
  {
    var name = 'Apple TV (4th generation)' + ' ' + this.state.storage
    const user = await AsyncStorage.getItem('@user:key')
    firebaseApp.database().ref('shopapp/list_User/').once('value', (childSnapshot) => {
      childSnapshot.forEach((child) => {
        if (user === child.toJSON().id)
  ...
  _onPress_StorageItem(item)
  {
    this.setState({
      ...
      name: 'Apple TV (4th generation)' + ' ' + item.storage
    })
  }

  _onPress_Gallery()
  {
    this.props.navigation.navigate("AppleTVGallery", {
      title: "Gallery"
    })
  }
  ...
  _onPress_Box()
  {
    this.props.navigation.navigate("AppleTVBox")
  }

  _onPress_TechSpecs()
  {
    this.props.navigation.navigate("AppleTVTechSpecs", {
      title: 'Tech Specs'
    })
  }
        ...
        <ScrollView
          ...
          <View
            ...
            <View
              ...
              <Text
                style={{paddingVertical: 2, fontFamily: 'System', fontSize: 32}}>Buy Apple TV</Text>
            </View>
            <Text
