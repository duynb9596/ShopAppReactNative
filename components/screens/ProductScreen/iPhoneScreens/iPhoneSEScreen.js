...
export default class iPhoneSEScreen extends React.Component {
  ...
  _loadData()
  {
    firebaseApp.database().ref('shopapp/list_ProductType/iPhone/product/iPhone SE/').once('value', (product) => {
      console.log(product)
      this.setState({
    ...
    firebaseApp.database().ref('shopapp/list_ProductType/iPhone/product/iPhone SE/price/').once('value', (price) => {
      this.setState({
        fromprice: parseInt(price.val()),
        storageprice: parseInt(price.val())
      })
    }).then(() => {
      firebaseApp.database().ref('shopapp/list_ProductType/iPhone/product/iPhone SE/model/list_Finish/').once('value', (childColourSnapshot) => {
      ...
      firebaseApp.database().ref('shopapp/list_ProductType/iPhone/product/iPhone SE/model/list_Storage/').once('value', (childStorageSnapshot) => {
        ...
        this.setState({
          name: 'iPhone SE' + ' ' + this.state.storage + ' ' + this.state.list_Finish.find(i => i.colour === this.state.colour).name
        })
      })
    })    
  }
  ...
  _writeData()
  {
    firebaseApp.database().ref('shopapp/list_ProductType/iPhone/product/iPhone SE/model/list_Finish/gold/image').set('https://firebasestorage.googleapis.com/v0/b/shopapp-8541a.appspot.com/o/iPhone%2FiPhoneSE-gold.png?alt=media&token=579e49a4-0d9c-48b2-857a-60267fff34a1')
    firebaseApp.database().ref('shopapp/list_ProductType/iPhone/product/iPhone SE/model/list_Finish/rosegold/image').set('https://firebasestorage.googleapis.com/v0/b/shopapp-8541a.appspot.com/o/iPhone%2FiPhoneSE-rosegold.png?alt=media&token=704cc268-7c29-4052-9aed-886200151b1a')
    firebaseApp.database().ref('shopapp/list_ProductType/iPhone/product/iPhone SE/model/list_Finish/grey/image').set('https://firebasestorage.googleapis.com/v0/b/shopapp-8541a.appspot.com/o/iPhone%2FiPhoneSE-spacegray.png?alt=media&token=6a92e169-5c5e-455b-ad81-8859f3d01228')
    firebaseApp.database().ref('shopapp/list_ProductType/iPhone/product/iPhone SE/model/list_Finish/silver/image').set('https://firebasestorage.googleapis.com/v0/b/shopapp-8541a.appspot.com/o/iPhone%2FiPhoneSE-silver.png?alt=media&token=7a52bf9c-4964-43c4-b324-adb4dc3288c6')
  }

  async _addToBag()
  {
    var name = 'iPhone SE' + ' ' + this.state.storage + ' ' + this.state.list_Finish.find(i => i.colour === this.state.colour).name
    const user = await AsyncStorage.getItem('@user:key')
    firebaseApp.database().ref('shopapp/list_User/').once('value', (childSnapshot) => {
      childSnapshot.forEach((child) => {
        if (user === child.toJSON().id)
  ...
  _onPress_FinishItem(colour)
  {
    this.setState({
      ...
      name: 'iPhone SE' + ' ' + this.state.storage + ' ' + this.state.list_Finish.find(i => i.colour === colour).name
    })
  }
  ...
  _onPress_StorageItem(item)
  {
    this.setState({
      ...
      name: 'iPhone SE' + ' ' + item.storage + ' ' + this.state.list_Finish.find(i => i.colour === this.state.colour).name
    })
  }

  _onPress_Gallery()
  {
    this.props.navigation.navigate("iPhoneSEGallery", {
      title: "Gallery"
    })
  }
  ...
  _onPress_Box()
  {
    this.props.navigation.navigate("iPhoneSEBox")
  }
      ...
      <SafeAreaView
        ...
        <ScrollView
          ...
          <View
            ...
            <Text
              style={{paddingTop: 20, paddingBottom: 10, fontFamily: 'System', fontSize: 32, marginTop: 20}}>Buy iPhone SE</Text>
            <Text
