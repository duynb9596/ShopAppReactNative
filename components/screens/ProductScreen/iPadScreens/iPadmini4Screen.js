...
export default class iPadmini4Screen extends React.Component {
  ...
  _loadData()
  {
    firebaseApp.database().ref('shopapp/list_ProductType/iPad/product/iPad mini 4/').once('value', (product) => {
      console.log(product)
      this.setState({
    ...
    firebaseApp.database().ref('shopapp/list_ProductType/iPad/product/iPad mini 4/price/').once('value', (price) => {
      this.setState({
        fromprice: parseInt(price.val()),
        storageprice: parseInt(price.val())
      })
    }).then(() => {
      firebaseApp.database().ref('shopapp/list_ProductType/iPad/product/iPad mini 4/model/list_Finish/').once('value', (childColourSnapshot) => {
      ...
      firebaseApp.database().ref('shopapp/list_ProductType/iPad/product/iPad mini 4/model/list_Storage/').once('value', (childStorageSnapshot) => {
      ...
      firebaseApp.database().ref('shopapp/list_ProductType/iPad/product/iPad mini 4/model/list_Connectivity/').once('value', (childConnectivitySnapshot) => {
        ...
        this.setState({
          name: 'iPad mini 4 ' + this.state.connectivity + ' ' + this.state.storage + ' - ' + this.state.list_Finish.find(i => i.colour === this.state.colour).name
        })
      })
    })   
  }
  ...
  _writeData()
  {
    firebaseApp.database().ref('shopapp/list_ProductType/iPad/product/iPad mini 4/model/list_Finish/gold/image').set('https://firebasestorage.googleapis.com/v0/b/shopapp-8541a.appspot.com/o/iPad%2FiPadmini4-gold.png?alt=media&token=b3f372b5-cf60-40f3-949e-2754b0220783')
    firebaseApp.database().ref('shopapp/list_ProductType/iPad/product/iPad mini 4/model/list_Finish/grey/image').set('https://firebasestorage.googleapis.com/v0/b/shopapp-8541a.appspot.com/o/iPad%2FiPadmini4-spacegray.png?alt=media&token=2d6aea8b-0152-4e2c-8b0a-8a5f58046987')
    firebaseApp.database().ref('shopapp/list_ProductType/iPad/product/iPad mini 4/model/list_Finish/silver/image').set('https://firebasestorage.googleapis.com/v0/b/shopapp-8541a.appspot.com/o/iPad%2FiPadmini4-silver.png?alt=media&token=d9eb2f38-999b-43e6-8a1b-dd2980a11209')
  }

  async _addToBag()
  {
    var name = 'iPad mini 4 ' + this.state.connectivity + ' ' + this.state.storage + ' - ' + this.state.list_Finish.find(i => i.colour === this.state.colour).name
    const user = await AsyncStorage.getItem('@user:key')
    firebaseApp.database().ref('shopapp/list_User/').once('value', (childSnapshot) => {
      childSnapshot.forEach((child) => {
        if (user === child.toJSON().id)
  ...
  _onPress_FinishItem(colour)
  {
    this.setState({
      ...
      name: 'iPad mini 4 ' + this.state.connectivity + ' ' + this.state.storage + ' - ' + this.state.list_Finish.find(i => i.colour === colour).name
    })
  }
  ...
  _onPress_StorageItem(item)
  {
    this.setState({
      ...
      name: 'iPad mini 4 ' + this.state.connectivity + ' ' + item.storage + ' - ' + this.state.list_Finish.find(i => i.colour === this.state.colour).name
    })
  }
  ...
  _onPress_ConnectivityItem(item)
  {
    this.setState({
      ...
      name: 'iPad mini 4 ' + item.connectivity + ' ' + this.state.storage + ' - ' + this.state.list_Finish.find(i => i.colour === this.state.colour).name
    })
  }

  _onPress_Gallery()
  {
    this.props.navigation.navigate("iPadmini4Gallery", {
      title: "Gallery"
    })
  }
  ...
  _onPress_Box()
  {
    this.props.navigation.navigate("iPadmini4Box")
  }
        ...
        <ScrollView
          ...
          <View
            ...
            <View
              ...
              <Text
                style={{paddingVertical: 2, fontFamily: 'System', fontSize: 32}}>iPad mini 4</Text>
            </View>
            <Text
          ...
          <View
            ...
            <FlatList
              ...
              numColumns={this.state.list_Storage > 1 ? 2 : 1}
              ItemSeparatorComponent={() => {
                return (
                  <View
                    style={{height: 10, width: dvWidth - 50, paddingHorizontal: 10}}
              ...
              renderItem={({item}) => {
                return(
                  <TouchableWithoutFeedback
                    ...
                    <View
                      style={{width: this.state.list_Storage > 1 ? (dvWidth - 50) / 2 : dvWidth - 40, height: 80, borderRadius: 4, borderColor: item.storage === this.state.storage ? 'blue' : '#666', borderWidth: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: 10, marginRight: 10}}>
                      <Text
